...
TextHandler::TextHandler(const char* filename) : filename(filename) {
    ...
    wordsStatistic = new std::map<std::string, int>();
    quantity = 0;
}
...
const std::map<std::string, int>& TextHandler::getWordsStatistic() const {
...
void TextHandler::insert(std::string word, std::set<std::string>* lst1) {
    lst1->insert(word);
}

void TextHandler::insert(std::string word, std::map<std::string, int>* lst1) {
    if ( lst1->find(word) == lst1->end() ) {
        lst1->insert(std::pair<std::string, int>(word, 0));
    }
    lst1->at(word) += 1;
}
...
std::ostream& operator<<(std::ostream& out, const TextHandler& handler) {
    ...
    out << "Matches in text: " << std::endl;
    out << handler.getWordsStatistic() << std::endl;
    return out;
